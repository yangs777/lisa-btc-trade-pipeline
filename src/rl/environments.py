...
class BTCTradingEnvironment(gym.Env):
    ...
    def __init__(
        ...
        self.feature_columns = [col for col in df.columns if col not in required_cols]
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        position_features = np.array(
            [
                (
                    self.position / (self.balance / current_price) if self.balance > 0 else 0
                ),  # Position ratio
    ...
    def get_episode_summary(self) -> dict[str, Any]:
        ...
        metrics["total_return_pct"] = (
            (final_equity - self.initial_balance) / self.initial_balance * 100
        )
